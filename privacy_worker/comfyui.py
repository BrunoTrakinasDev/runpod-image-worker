...
class ComfyUIProcessManager:
    ...
    def _is_ready(self) -> bool:
        try:
            response = requests.get(f"{self.settings.comfyui_base_url}/system_stats", timeout=3)
            return response.ok
    ...
    def ensure_started(self, request_id: str) -> None:
        ...
        while time.monotonic() < deadline:
            if self._process and self._process.poll() is not None:
                raise ComfyUIError("ComfyUI encerrou durante a inicialização.", details={"returncode": self._process.returncode})
            if self._is_ready():
...
class ComfyUIClient:
    ...
    def wait_for_history(self, prompt_id: str, request_id: str) -> dict[str, Any]:
        ...
        while time.monotonic() < deadline:
            try:
                response = requests.get(f"{self.settings.comfyui_base_url}/history/{prompt_id}", timeout=20)
                response.raise_for_status()
                payload = response.json()
            except (requests.RequestException, json.JSONDecodeError) as error:
                log_event("comfyui_history_retry", request_id=request_id, level="WARN", prompt_id=prompt_id, error=str(error))
            ...
            if record:
                ...
                if status_str == "error":
                    raise ComfyUIError("ComfyUI falhou ao executar o workflow.", details={"messages": messages[-5:]})
                if completed or record.get("outputs"):
                    log_event("comfyui_prompt_completed", request_id=request_id, prompt_id=prompt_id, elapsed_ms=now_ms() - started)
                    return record
            time.sleep(self.settings.comfyui_poll_interval_seconds)
    ...
    def _candidate_entries(record: dict[str, Any], output_nodes: tuple[str, ...]):
        ...
        for node_id in ordered_nodes:
            ...
            entries = node_output.get("images") or []
            for entry in reversed(entries if isinstance(entries, list) else []):
                if isinstance(entry, dict) and entry.get("filename"):
                    yield entry

    def download_output(self, *, record: dict[str, Any], output_nodes: tuple[str, ...], destination: Path, request_id: str) -> Path:
        candidate = next(self._candidate_entries(record, output_nodes), None)
        if not candidate:
            raise OutputError("O workflow terminou sem arquivo de imagem reconhecível.")
        query = urlencode({
        ...
        downloaded = 0
        try:
            with requests.get(f"{self.settings.comfyui_base_url}/view?{query}", stream=True, timeout=(15, 600)) as response:
                response.raise_for_status()
                suffix = Path(candidate["filename"]).suffix or ".png"
                ...
                with output_path.open("wb") as handle:
                    for chunk in response.iter_content(chunk_size=1024 * 1024):
                        ...
                        if downloaded > max_bytes:
                            raise OutputError(f"Saída do ComfyUI excede MAX_OUTPUT_MB={self.settings.max_output_mb}.")
                        handle.write(chunk)
        except OutputError:
            raise
        except Exception as error:
            raise OutputError("Falha ao baixar a imagem gerada pelo ComfyUI.") from error
        log_event("comfyui_output_downloaded", request_id=request_id, size_bytes=downloaded, destination=str(output_path))
        return output_path
